...
{
    if (!_rightItem) {
        _rightItem = [LYSDatePickerItem btnWithTitle:@"完成" target:self action:@selector(commitAction:)];
    }
    return _rightItem;
}
...
{
    if (!_centerItem) {
        _centerItem = [LYSDatePickerItem labelWithTitle:@"选择时间"];
    }
    return _centerItem;
}
...
- (void)setHeaderHeight:(CGFloat)headerHeight
{
    ...
    rightItemFrame.origin.x = [UIScreen mainScreen].bounds.size.width - CGRectGetWidth(rightItemFrame) - 16;
